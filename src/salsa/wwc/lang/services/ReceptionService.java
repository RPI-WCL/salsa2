...
public class ReceptionService implements Runnable {
    ...
    public ReceptionService() {
        while(!startService(this.port))
            this.port++;
    }

    public ReceptionService(int port) {
        this.port = port;
        startService(this.port);
    }
    ...
    public boolean startService(int port) {
        ...
        try {
            ...
            logger.info("Reception Service listening on: " + host + ":" + port);
            rev = true;
        } catch (IOException e) {
    ...
    public boolean isLocal(String host, int port) {
        return (this.host.equalsIgnoreCase(host)
                ...
                || host.equals("127.0.0.1") || host.equals("127.0.1.1"))
                && this.port == port;
    }
                ...
                try {
                    ...
                    while (true) {
                        try {
                            ...
                            // receivedObject =
                            // ExternalizableHelper.fromByteArray(b,
                        ...
                        if (receivedObject != null) {
                            if (receivedObject instanceof ActorState) {
                                ...
                                ServiceFactory.getStageService()
                                        .registerIncomingActor(actorState);
